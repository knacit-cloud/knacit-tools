...
- ✅【人間確認済 2026-07-27】A〜E2 の6行を一次で確認済み（下の台帳）。**種別の注意**：Aは意見コラム・Cは社説（どちらも証拠ではない）／**B は Neuron の Perspective**（Reviewではない）・**D は Biosystems の Review article**。**「原理的」を支えるのは実質D 1本**。**B・Dとも実はオープンアクセス**（当初「全てペイウォール」としていたのは誤り）。**残る未確認は F のみ**（Huddersfield大に無料の著者版あり）。
